...
func (h *GameHandler) SettlementMiniProgramCode(w http.ResponseWriter, r *http.Request) {
	image, err := h.game.SettlementMiniProgramCode(r.Context(), middleware.UserID(r.Context()), chi.URLParam(r, "id"))
	if err != nil {
		response.Error(w, err)
		return
	}
